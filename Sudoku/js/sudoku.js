...
$(document).ready(function () {
    ...
    height = (height - 698) / 2 - 34;
    $("#Container").css("margin-top", height+50);
	initBoard('Container');
	makeBoard();
});
...
function Space() {
	...
	this.setValue = function(i) {
		if (i < 0 || i > 9)
		    Windows.UI.Popups.MessageDialog(i, "").showAsync();
		...
		for (var x = 0; x < 9; ++x) {
			...
			a.data("x", x)
				...
				.click(function(e) {
				    ...
				    keyboard_shown = true;
					//Keyboard position
					var canPos = WinJS.Utilities.getPosition(e.currentTarget);
				    ...
				    // $("#keyboard").text($("#clearBoard").css("right"));
				    
				    $("#keyboard").css("left",docWidth-400);
					$("#keyboard").css("top",docHeight/2-200);
					$("#keyboard").fadeIn();
...
function solveBoard() {
	...
	Windows.UI.Popups.MessageDialog("Try hitting \"Reset\" to remove your moves.", "This board has no solutions").showAsync();
	board.render();
}
...
function insert_value(value) {
    x = $(".Sel").data("x");
    y = $(".Sel").data("y");
    board.setValue(x, y, parseInt(value));
